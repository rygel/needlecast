...
class MenuBarBuilder(
    ...
    private fun buildFileMenu(i18n: io.github.rygel.outerstellar.i18n.I18nService): JMenu {
        val settingsItem = JMenuItem(i18n.translate("menu.file.settings")).apply {
            addActionListener {
                SettingsDialog(
                    ...
                    callbacks = coordinator.createSettingsCallbacks(
        ...
        return JMenu(i18n.translate("menu.file")).apply {
            ...
            addSeparator()
            add(importLayoutItem); add(exportLayoutItem)
            addSeparator()
            add(exitItem)
        }
    }
    ...
    private fun buildAiMenu(): JMenu {
        ...
        val promptLibraryItem = JMenuItem("Prompt Library...").apply {
            addActionListener {
                PromptLibraryDialog(owner = owner, ctx = ctx,
    ...
    private fun showAbout() {
        ...
        val linkLabel = JLabel("<html><a href=''>$repoUrl</a></html>").apply {
            cursor = Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)
            addMouseListener(object : MouseAdapter() {
                override fun mouseClicked(e: MouseEvent) {
                    try { Desktop.getDesktop().browse(URI(repoUrl)) } catch (_: Exception) {}
                }
            })
        }
        ...
        val content = JPanel(GridBagLayout()).apply {
            val gbc = GridBagConstraints().apply {
                gridx = 0; gridy = 0; insets = Insets(0, 0, 12, 0)
                anchor = GridBagConstraints.CENTER
            }
            add(JLabel("Needlecast $version", SwingConstants.CENTER).apply {
                font = font.deriveFont(Font.BOLD, 16f)
            }, gbc)
            gbc.gridy++; gbc.insets = Insets(0, 0, 4, 0)
            add(JLabel("A project launcher for developers"), gbc)
            gbc.gridy++; gbc.insets = Insets(0, 0, 12, 0)
            add(JLabel("by Alexander Brandt"), gbc)
            gbc.gridy++; gbc.insets = Insets(0, 0, 4, 0)
            add(linkLabel, gbc)
            gbc.gridy++; gbc.insets = Insets(8, 0, 0, 0)
            add(JLabel("<html><center>MIT License<br>Java ${System.getProperty("java.version")}</center></html>",
                SwingConstants.CENTER).apply {
                foreground = java.awt.Color.GRAY
            }, gbc)
        }

        JOptionPane.showMessageDialog(owner, content, "About Needlecast",
