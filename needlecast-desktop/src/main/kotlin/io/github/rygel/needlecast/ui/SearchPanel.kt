...
class SearchPanel(
    ...
    private fun startSearch() {
        ...
        val includeMatchers =
            try {
                buildMatchers(includeField.text)
        ...
        val excludeMatchers =
            try {
                buildMatchers(excludeField.text, stripNegation = true)
            } catch (e: IllegalArgumentException) {
    ...
    private fun shouldSkipDir(name: String?): Boolean = name != null && SKIP_DIRS.contains(name.lowercase())
    ...
    private fun formatSummary(stats: SearchStats): String {
        ...
        return if (stats.truncated) {
            "$base$extra — results capped at $MAX_RESULTS."
        } else {
            "$base$extra"
        }
    }
    ...
    companion object {
        ...
        private val SKIP_FILE_EXTENSIONS =
            setOf(
                "class",
                "jar",
                "war",
