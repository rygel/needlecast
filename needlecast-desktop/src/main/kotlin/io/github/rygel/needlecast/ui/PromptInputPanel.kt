...
class PromptInputPanel(
    ...
    private val editButton =
        JButton(RemixIcons.icon("ri-edit-line", 16)).apply {
            isFocusable = false
            isEnabled = false
            margin = Insets(1, 5, 1, 5)
        }
    private val deleteButton =
        JButton(RemixIcons.icon("ri-delete-bin-line", 16)).apply {
            isFocusable = false
            isEnabled = false
            margin = Insets(1, 5, 1, 5)
        }
    ...
    private fun refreshTree(templates: List<PromptTemplate>) {
        ...
        byCategory.keys
            .sortedWith(compareBy { if (it == "Uncategorized") "\uFFFF" else it })
            .forEach { cat ->
