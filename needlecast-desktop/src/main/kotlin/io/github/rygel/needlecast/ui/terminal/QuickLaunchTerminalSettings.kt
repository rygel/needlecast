...
class QuickLaunchTerminalSettings(
    ...
    private fun makePalette(ansi: Array<Color>): ColorPalette =
        object : ColorPalette() {
            override fun getForegroundByColorIndex(index: Int): Color = if (index in ansi.indices) ansi[index] else Color.WHITE

            override fun getBackgroundByColorIndex(index: Int): Color = if (index in ansi.indices) ansi[index] else Color.BLACK
        }
    ...
    override fun getSelectionColor(): TextStyle {
        val selBg =
            javax.swing.UIManager.getColor("TextArea.selectionBackground")
                ?: if (isDark) Color(0x264F78) else Color(0xADD6FF)
        val selFg =
            javax.swing.UIManager.getColor("TextArea.selectionForeground")
