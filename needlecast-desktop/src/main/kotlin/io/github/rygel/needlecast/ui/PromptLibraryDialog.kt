...
class PromptLibraryDialog(
    ...
    private fun saveCurrentForm() {
        ...
        for (i in 0 until listModel.size) {
            if (listModel.getElementAt(i).id == updated.id) {
                promptList.selectedIndex = i
                break
            }
        }
    }
    ...
    private fun populateList(templates: List<PromptTemplate>) {
        ...
        if (previousId != null) {
            for (i in 0 until listModel.size) {
                if (listModel.getElementAt(i).id == previousId) {
                    promptList.selectedIndex = i
                    break
                }
            }
