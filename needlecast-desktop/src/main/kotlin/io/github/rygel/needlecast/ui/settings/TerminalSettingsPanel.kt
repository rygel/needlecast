...
class TerminalSettingsPanel(
    ...
    private val shellField =
        JTextField(ctx.config.defaultShell ?: "", 28).apply {
    ...
    init {
        ...
        val terminalCombo =
            JComboBox(monoChoices.toTypedArray()).apply {
        ...
        add(
            JLabel("<html><i>Takes effect on next terminal activation.</i></html>").apply {
        ...
        val resetColorsBtn =
            JButton("Reset").apply {
                ...
                addActionListener {
                    currentFgColor = null
                    currentBgColor = null
                    fgSwatch.background = Color.GRAY
                    bgSwatch.background = Color.GRAY
                    saveAndApply()
                }
            }
        ...
        gc.insets = Insets(0, 4, 4, 4)
        add(
            JLabel("<html><i>Click a swatch to pick a color. Takes full effect in new terminal tabs.</i></html>").apply {
                font = font.deriveFont(Font.PLAIN, font.size2D - 1f)
            },
            gc,
        )
