...
class TerminalPtyPlatformTest {
    ...
    fun `shell command resolves to bash on unix`() {
        ...
        assertFalse(
            cmd.any { it.contains("cmd.exe", ignoreCase = true) },
    ...
    fun `shouldConsumeRemoteMouseWheelEvent ignores local wheel`() {
        ...
        assertFalse(
            shouldConsumeRemoteMouseWheelEvent(event, isRemoteMouseAction = false),
    ...
    private fun invokeResolveShellCommand(shellExecutable: String?): Array<String> {
        val constructor =
            TerminalPanel::class.java.getDeclaredConstructor(
                String::class.java,
